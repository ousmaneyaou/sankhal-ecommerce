...
const AdminOrdersPage = () => {
    ...
    const fetchOrders = async () => {
        ...
        try {
            ...
            setError(error.response?.data?.message || error.message || 'incapacité à obtenir des commandes')
            setTimeout(()=>{
                setError('')
            }, 3000)
        }
                ...
                <tbody>
                    {filteredOrders.map(order => (
                        <tr key={order.id}>
                            ...
                            <td>{order.status}</td>
                            <td>{order.price.toFixed(2)} XOF</td>
                            <td>{order.createdAt ? new Date(Number(order.createdAt)).toLocaleDateString() : "N/A"}</td>
                            <td>
